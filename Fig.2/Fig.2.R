...
ggplot(data_m, aes(x=COG, y=value),color=COG) + 
  ...
  theme( axis.title.x = element_text(size=15),axis.title.y = element_text(size=15),
         ...
         legend.text = element_text(size=10),legend.title = element_blank())+
  theme(legend.position='none')+guides(fill = guide_legend(ncol = 2, byrow = TRUE))
ggsave("Fig. 2a1.tiff",width=7,height=4.3, path="D:/")  
...
ggplot(data, mapping = aes(x=factor,y=gene))+
  geom_point(fill="lightgray",size = 3.2, alpha = 0.85,shape=21)+
  labs(x="Total organic carbon (g/kg)",y="Abundance of methane oxidation genes")+
  theme(axis.line = element_line(color="black"))+
  geom_smooth(color="#95D1D7",method = 'lm',se=T,size=1.2,fullrange=T,fill="lightgray",alpha=0.25)+
  ...
  stat_poly_eq(aes(label = paste(..adj.rr.label.., ..p.value.label.., sep = "~`,`~")),size=5,
           label.x = )
ggsave("Fig.2d.tiff", width= 4.6,height= 4.1,path="D:/")      
#SO42--sulphate reduction
data<-read_excel("Fig.2.xlsx") 
ggplot(data, mapping = aes(x=factor,y=gene))+
  geom_point(fill="lightgray",size = 3.3, alpha = 0.85,shape=21)+
  ...
  stat_poly_eq(aes(label = paste(eq.label,..adj.rr.label.., ..p.value.label.., sep = "~`,`~")),size=4.7,
               label.x = )
ggsave("Fig.S1A.tiff", width= 5,height= 5,path="D:/") 
#TP-phoh
data<-read_excel("Fig.2.xlsx")
ggplot(data, mapping = aes(x=factor,y=gene))+
  ...
  stat_poly_eq(aes(label = paste(eq.label,..adj.rr.label.., ..p.value.label.., sep = "~`,`~")),size=4.7,
               label.x = )
ggsave("Fig.S1B.tiff", width= 5,height= 5,path="D:/") 
