...
map<-ggplot()+
  ...
  theme_test()+ 
  theme(aspect.ratio = 0.95,axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank(),
        panel.grid = element_blank(),panel.border = element_rect(fill=NA,color="white",linetype=1))+
...
ggplot() +geom_sf(data = HN,color='black',fill="#FCFCFC",size=2)+geom_sf(data = JS,color='#5B5B5B',fill="#FCFCFC",size=0.5)+geom_sf(data = ZJ,color='#5B5B5B',fill="#FCFCFC",size=0.5)+
  ...
  theme(plot.background = element_rect(fill="white",
                                       color="white",size = 0.5))+geom_point(data=mydata,aes(x=Latitude,y=Longitude),color="#FF8040",size=1.5,shape=15)
#Fig. 1b
sales <- c(20.3,79.7)
names<-c("a","b")
...
ggsave("Fig.1b2.tiff",width=5,height=5,path="D:/")
...
ggplot(data,aes(x=letter, y=ratio))+geom_violin(aes(fill=letter),trim=F,width=0.3,alpha=0.9)+geom_boxplot(width=0.18)+
  labs(x="",y="Relative proportion (%)")+geom_jitter(aes(x=letter, y=ratio,fill=letter),shape=21,size=1.7,position=position_jitterdodge(jitter.width = 0.4),alpha=0.8)+
...
#Fig.s1a
data<-read_excel("Fig.1c.xlsx")
data$Type <- factor(data$Type,level=c('DD','DY','QD','LYG','YC','NB','WZ','XM','ST','ZH','BH','SY'))
data$Lifestyle = factor(data$Lifestyle, levels=c("Lytic","Lysogenic"))
ggplot(data)+geom_boxplot(aes(x=Type, y=Lifestyles,fill=Lifestyle),alpha=0.85,width=1,outliers = F)+
  labs(x="",y="Relative proportion (%)")+geom_jitter(aes(x=Type, y=Lifestyles,fill=Lifestyle),shape=21,size=1.2,position=position_jitterdodge(jitter.width = 0.2),alpha=0.7,inherit.aes=F)+
  scale_fill_manual(values = c("#7AC5CD","#FF9B64"))+scale_color_manual(values = c("#7AC5CD","#FF9B64"))+
  theme(axis.line = element_line(color="black"))+
  theme_test()+ theme(panel.grid.major=element_blank(),panel.grid.minor = element_blank(),
                      axis.title.x = element_text(size=18.5),axis.title.y = element_text(size=18.5),
                      axis.text.x = element_text(hjust =0.5,size=16,colour = 'black'),
...
#Fig.s1b
data<-read_excel('Fig.1c.xlsx')
ggplot(data, mapping = aes(x=Microbe,y=Lifestyles))+
  geom_point(aes(fill=Lifestyle),size = 3.2, alpha = 0.85,shape=21)+
  labs(x="Normalized microbial abundance",y="Relative proportion (%)")+
  ...
  theme(panel.grid.major=element_blank(),panel.grid.minor = element_blank(),
        ...
        legend.text = element_text(size=15),
        legend.title = element_text(size=16))+
  ylim(20,80)+ scale_x_continuous(breaks = seq(0,12000,2000))+xlim(1500,11900)
...
ggplot(all_new)+geom_point(aes(x=sites.1,y=richness.1),shape=19,size=2.5,color="#7AC5CD",alpha=0.5)+
  ...
  geom_errorbar(aes(sites.2,ymin=richness.2-sd.2,ymax=richness.2+sd.2),lwd=0.7,width=1,alpha=0.8,position=position_dodge(0),
                color="gray")+
  geom_point(aes(x=sites,y=richness),shape=19,size=2.5,color="#FF9B64",alpha=0.5)+
